...
/*   Updated: 2025/04/01 15:22:49 by hmunoz-g         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
export class PhysicsSystem {
    ...
    update(entities, delta) {
        ...
        if (ball && paddleL && paddleR) {
            this.updateBall(ball, paddleL, paddleR);
        }
    ...
    updateBall(ball, paddleL, paddleR) {
        ...
        if (physics.rotationSpeed === undefined) {
            physics.rotationSpeed = 0.1; // Adjust this value for desired rotation speed
        }
        ...
        // Positive velocity (moving right) = clockwise rotation (positive)
        // Negative velocity (moving left) = counter-clockwise rotation (negative)
        ...
        if (ballTop <= 0) {
            physics.velocityY = Math.abs(physics.velocityY);
            physics.y = physics.height / 2;
        } else if (ballBottom >= this.height) {
            physics.velocityY = -Math.abs(physics.velocityY);
            physics.y = this.height - physics.height / 2;
        }
