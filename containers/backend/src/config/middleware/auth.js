...
async function verifyToken(request, reply) {
	...
	// If no token in header, try query parameter (for avatar requests)
	if (!token && request.query && request.query.token) {
		token = request.query.token;
		console.log('[middleware/auth] token from query:', token ? 'PRESENT' : 'MISSING');
	}

	// If no token in header or query, fall back to session
	if (!token) {
		token = request.session.get('token');
		console.log('[middleware/auth] token from session:', token ? 'PRESENT' : 'MISSING');
	}
	...
	console.log('[middleware/auth] Query params:', request.query);
	console.log('[middleware/auth] Final token:', token ? 'PRESENT' : 'MISSING');

	if (!token) {
		console.log('[middleware/auth] No token found in any source');
		return reply.status(401).send({
			success: false,
			message: 'Authentication required - no token provided',
		});
